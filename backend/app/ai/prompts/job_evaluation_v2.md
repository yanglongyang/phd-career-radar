...
3b. **reputation 维度只能使用 `eligible_for_reputation_scoring=true` 的 Evidence**；
    `false` 的证据可以作为其他维度/风险/信息缺口的参考，但不得影响 reputation 分；
    不存在 true Evidence 时，`reputation` 必须为 null（v2 新增契约）。
4. 只输出一个合法 JSON 对象。
