/* V0.3 → V0.3.3 sector：来源/单位性质分类的展示派生（纯函数，便于单测）。
   医院（hospital）是独立 sector；科研院所（research_institute）是独立 sector。
   sector 只回答"单位性质"，不承载研究方向（那是未来 domain_tags 的职责）。 */
...
export const SECTOR_TABS: { key: string; label: string }[] = [
  ...
  { key: "research_institute", label: "科研院所" },
...
/* 运行结果按 sector 分组（前端 presentation grouping，不改事务/统计语义）。
   顺序固定：高校 → 科研院所 → 医院 → 央国企 → 企业 → 混合 → 其他；未知归入其他。 */
export function groupRunItemsBySector<T extends { sector?: string | null }>(
  ...
  const order = [
    ...
    "state_owned", "enterprise", "mixed", "other",
  ];
